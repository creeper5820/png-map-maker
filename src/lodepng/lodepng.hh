...
/*pass -DLODEPNG_NO_COMPILE_PNG to the compiler to disable this, or comment out LODEPNG_COMPILE_PNG
 * below*/
...
/*pass -DLODEPNG_NO_COMPILE_DECODER to the compiler to disable this, or comment out
 * LODEPNG_COMPILE_DECODER below*/
#  define LODEPNG_COMPILE_DECODER
# endif

/*deflate&zlib encoder and png encoder*/
# ifndef LODEPNG_NO_COMPILE_ENCODER
/*pass -DLODEPNG_NO_COMPILE_ENCODER to the compiler to disable this, or comment out
 * LODEPNG_COMPILE_ENCODER below*/
...
The default built-in CRC code comes with 8KB of lookup tables, so for memory constrained environment
you may want it disabled and provide a much smaller implementation externally as said above. You can
find such an example implementation in a comment in the lodepng.c(pp) file in the 'else' case of the
searchable LODEPNG_COMPILE_CRC section.*/
...
to handle such files and encode in-memory.*/
unsigned
    lodepng_encode32_file(const char* filename, const unsigned char* image, unsigned w, unsigned h);
...
to handle such files and encode in-memory.*/
unsigned
    lodepng_encode24_file(const char* filename, const unsigned char* image, unsigned w, unsigned h);
#   endif /*LODEPNG_COMPILE_DISK*/
...
struct LodePNGDecompressSettings {
    ...
    unsigned ignore_adler32; /*if 1, continue and don't give an error message if the Adler32
    ...
    ignored by the PNG decoder, but is used by the deflate/zlib decoder and can be used by custom
    ones. Set to 0 to impose no limit (the default).*/
...
struct LodePNGCompressSettings /*deflate = compress*/ {
    ...
    unsigned btype;      /*the block type for LZ (0, 1, 2 or 3, see zlib standard). Should be 2 for
                            proper compression.*/
    unsigned use_lz77;   /*whether or not to use LZ77. Should be 1 for proper compression.*/
    unsigned windowsize; /*must be a power of two <= 32768. higher compresses more but is slower.
                            Default value: 2048.*/
    unsigned minmatch;   /*minimum lz77 length. 3 is normally best, 6 can be better for some PNGs.
                            Default: 0*/
    unsigned nicematch;  /*stop searching if >= this length found. Set to 258 for best compression.
                            Default: 128*/
...
typedef struct LodePNGColorMode {
    ...
    unsigned
        bitdepth; /*bits per sample, see PNG standard or documentation further in this header file*/
    ...
    */
    unsigned char* palette; /*palette in RGBARGBA... order. Must be either 0, or when allocated must
                               have 1024 bytes*/
    size_t
        palettesize; /*palette size in number of colors (amount of used bytes is 4 * palettesize)*/

    /*
    transparent color key (tRNS)

    This color uses the same bit depth as the bitdepth value in this struct, which can be 1-bit to
    16-bit. For grayscale PNGs, r, g and b will all 3 be set to the same.
...
/*
Check if the given color info indicates the possibility of having non-opaque pixels in the PNG
image. Returns true if the image can have translucent or invisible pixels (it still be opaque if it
doesn't use such pixels). Returns false if the image can only have opaque pixels. In detail, it
returns true only if it's a color type with alpha, or has a palette with non-opaque values, or if
"key_defined" is true.
*/
...
#  ifdef LODEPNG_COMPILE_ANCILLARY_CHUNKS
    ...
    */
    ...
    All the string fields below including strings, keys, names and language tags are null
    terminated. The PNG specification uses null characters for the keys, names and tags, and forbids
    null characters to appear in the main text which is why we can use null termination everywhere
    here.
    ...
    */
    size_t
        text_num; /*the amount of texts in these char** buffers (there may be more texts in itext)*/
    ...
    */
    ...
    char** itext_langtags; /*language tag for this text's language, ISO/IEC 646 string, e.g. ISO 639
                              language tag*/
    ...
    The exif data is in exif-encoded form but without JPEG markers, starting with the 'II' or 'MM'
    marker that indicates endianness. It's up to an exif handling library to encode/decode its
    information.
    */
    unsigned exif_defined; /* Whether exif metadata is present, that is, the PNG image has an eXIf
                              chunk */
    ...
    /*phys chunk (pHYs)*/
    unsigned phys_defined; /*if 0, there is no pHYs chunk and the values below are undefined, if 1
                              else there is one*/
    ...
    LodePNG does not apply any color conversions on pixels in the encoder or decoder and does not
    interpret these color profile values. It merely passes on the information. If you wish to use
    color profiles and convert colors, a separate color management library should be used. There is
    also a limited library for this in lodepng_util.h.

    There are 4 types of (sets of) chunks providing color information. If multiple are present, each
    will be decoded by LodePNG, but only one should be handled by the user, with the following order
    of priority depending on what the user supports: 1: cICP: Coding-independent code points (CICP)
    2: iCCP: ICC profile
    ...
    */
    ...
    unsigned srgb_intent;  /* Rendering intent: 0=perceptual, 1=rel. colorimetric, 2=saturation,
                              3=abs. colorimetric */
    ...
    Contains ICC profile, which can use any version of the ICC.1 specification by the International
    Color Consortium. See its specification for more details. LodePNG does not parse or use the ICC
    profile (except its color space header field for "RGB" or "GRAY", see below), a separate library
    to handle the ICC data format is needed to use it for color management and conversions.

    For encoding, if iCCP is present, the PNG specification recommends to also add gAMA and cHRM
    chunks that approximate the ICC profile, for compatibility with applications that don't use the
    ICC chunk. This is not required, and it's up to the user to compute approximate values and set
    then in the appropriate gama_ and chrm_ fields, LodePNG does not do this automatically since it
    does not interpret the ICC profile.

    For encoding, the ICC profile is required by the PNG specification to be an "RGB" profile for
    non-gray PNG color types (types 2, 3 and 6) and a "GRAY" profile for gray PNG color types (types
    1 and 4). If you disable auto_convert, you must ensure the ICC profile type matches your
    requested color type, else the encoder gives an error. If auto_convert is enabled (the default),
    and the ICC profile is not a correct match for the pixel data, this will result in an encoder
    error if the pixel data has non-gray pixels for a GRAY profile, or a silent less-optimal
    compression of the pixel data if the pixels could be encoded as grayscale but the ICC profile is
    RGB.

    To avoid this do not set an ICC profile in the image unless there is a good reason for it, and
    when doing so make sure you compute it carefully to avoid the above problems.
    */
    ...
    */
    ...
    If defined, these values give the bit depth of the original data. Since PNG only stores 1, 2, 4,
    8 or 16-bit per channel data, the significant bits value can be used to indicate the original
    encoded data has another sample depth, such as 10 or 12.
    ...
    For example, setting these fields to 10-bit will force the encoder to keep using a 16-bit per
    channel color mode, even if the pixel data would in fact fit in a more efficient 8-bit mode.
    */
    ...
    For encoding, do not store critical chunks or known chunks that are enabled with a "_defined"
    flag above in here, since the encoder will blindly follow this and could then encode an invalid
    PNG file (such as one with two IHDR chunks or the disallowed combination of sRGB with iCCP). But
    do use this if you wish to store an ancillary chunk that is not supported by LodePNG (such as
...
unsigned lodepng_add_text(
    LodePNGInfo* info, const char* key, const char* str); /*push back both texts at once*/
void lodepng_clear_text(
    LodePNGInfo* info); /*use this to clear the texts again after you filled them in*/
...
void lodepng_clear_itext(
    LodePNGInfo* info); /*use this to clear the itexts again after you filled them in*/
...
void lodepng_clear_icc(
    LodePNGInfo* info); /*use this to clear the profile again after you filled it in*/

/*replaces if exists*/
unsigned lodepng_set_exif(LodePNGInfo* info, const unsigned char* exif, unsigned exif_size);
void lodepng_clear_exif(
    LodePNGInfo* info); /*use this to clear the exif metadata again after you filled it in*/
#  endif                /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/

/*
Converts raw buffer from one color type to another color type, based on
LodePNGColorMode structs to describe the input and output color type.
See the reference manual at the end of this header file to see which color conversions are
supported. return value = LodePNG error code (0 if all went ok, an error if the conversion isn't
supported) The out buffer must have size (w * h * bpp + 7) / 8, where bpp is the bits per pixel of
the output color type (lodepng_get_bpp). For < 8 bpp images, there should not be padding bits at the
end of scanlines. For 16-bit per channel colors, uses big endian format like PNG does. Return value
is LodePNG error code
*/
...
typedef struct LodePNGDecoderSettings {
    ...
    unsigned ignore_end; /*ignore issues at end of file if possible (missing IEND chunk, too large
                            chunk, ...)*/
    /* TODO: make a system involving warnings with levels and a strict mode instead. Other
       potentially recoverable errors: srgb rendering intent value, size of content of ancillary
       chunks, more than 79 characters for some strings, placement/combination rules for ancillary
       chunks, crc of unknown chunks, allowed characters in string keys, invalid characters in chunk
       types names, etc... */

    unsigned color_convert; /*whether to convert the PNG to the color type you want. Default: yes*/

#   ifdef LODEPNG_COMPILE_ANCILLARY_CHUNKS
    unsigned read_text_chunks; /*if false but remember_unknown_chunks is true, they're stored in the
                                  unknown chunks*/

    /*store all bytes from unknown chunks in the LodePNGInfo (off by default, useful for a png
     * editor)*/
    unsigned remember_unknown_chunks;

    /* maximum size for decompressed text chunks. If a text chunk's text is larger than this, an
    error is returned, unless reading text chunks is disabled or this limit is set higher or
    disabled. Set to 0 to allow any size. By default it is a value that prevents unreasonably large
    strings from hogging memory. */
    size_t max_text_size;

    /* maximum size for compressed ICC chunks. If the ICC profile is larger than this, an error will
    be returned. Set to 0 to allow any size. By default this is a value that prevents ICC profiles
    that would be much larger than any legitimate profile could be to hog memory. */
    size_t max_icc_size;
#   endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/
...
typedef enum LodePNGFilterStrategy {
    ...
    LFS_THREE = 3,
    LFS_FOUR  = 4,
    /*Use the filter out of the 5 above types that gives minimum sum, by trying each one. This is
    the adaptive filtering suggested heuristic in the PNG standard chapter 'Filter selection'.*/
    LFS_MINSUM,
...
/*Gives characteristics about the integer RGBA colors of the image (count, alpha channel usage, bit
depth, ...), which helps decide which color model to use for encoding. Used internally by default if
"auto_convert" is enabled. Public because it's useful for custom algorithms.*/
typedef struct LodePNGColorStats {
    unsigned colored;     /*not grayscale*/
    unsigned key;         /*image is not opaque and color key is possible instead of full alpha*/
    unsigned short key_r; /*key values, always as 16-bit, in 8-bit case the byte is duplicated, e.g.
                             65535 means 255*/
    unsigned short key_g;
    unsigned short key_b;
    unsigned alpha;       /*image is not opaque and alpha channel or alpha palette required*/
    unsigned numcolors;   /*amount of colors, up to 257. Not valid if bits == 16 or allow_palette is
                             disabled.*/
    unsigned char palette[1024]; /*Remembers up to the first 256 RGBA colors, in no particular
                                    order, only valid when numcolors is valid*/
    unsigned bits; /*bits per channel (not for palette). 1,2 or 4 for grayscale only. 16 if 16-bit
                      per channel required.*/
    size_t numpixels;

    /*user settings for computing/using the stats*/
    unsigned allow_palette;   /*default 1. if 0, disallow choosing palette colortype in
                                 auto_choose_color, and don't count numcolors*/
    unsigned allow_greyscale; /*default 1. if 0, choose RGB or RGBA even if the image only has gray
                                 colors*/
} LodePNGColorStats;
...
unsigned lodepng_compute_color_stats(
    LodePNGColorStats* stats, const unsigned char* image, unsigned w, unsigned h,
...
typedef struct LodePNGEncoderSettings {
    ...
    /*If true, follows the suggestion in the PNG standard in chapter 'Filter selection': if the PNG
    uses a palette or lower than 8 bit depth, set all filters to zero. In other cases this will use
    the heuristic from the chosen filter_strategy. The PNG standard suggests LFS_MINSUM for those
    cases.*/
...
/*Finds the first chunk with the given type in the range [chunk, end), or returns NULL if not
 * found.*/
...
#  ifdef LODEPNG_COMPILE_DECODER
/*Inflate a buffer. Inflate is the decompression step of deflate. Out buffer must be freed after
 * use.*/
...
unsigned encode(
    std::vector<unsigned char>& out, const unsigned char* in, unsigned w, unsigned h, State& state);
unsigned encode(
    std::vector<unsigned char>& out, const std::vector<unsigned char>& in, unsigned w, unsigned h,
    State& state);
#   endif /*LODEPNG_COMPILE_ENCODER*/
...
/*
TODO:
[.] test if there are no memory leaks or security exploits - done a lot but needs to be checked
often
[.] check compatibility with various compilers  - done but needs to be redone for every newer
version [X] converting color to 16-bit per channel types [X] support color profile chunk types (but
never let them touch RGB values by default) [ ] support all second edition public PNG chunk types
(almost done except sPLT and hIST) [X] support non-animation third edition public PNG chunk types:
eXIf, cICP, mDCV, cLLI [ ] make sure encoder generates no chunks with size > (2^31)-1 [ ] partial
decoding (stream processing) [X] let the "isFullyOpaque" function check color keys and transparent
palettes too [X] better name for the variables "codes", "codesD", "codelengthcodes", "clcl" and
"lldl" [ ] allow treating some errors like warnings, when image is recoverable (e.g. 69, 57, 58) [ ]
make warnings like: oob palette, checksum fail, data after iend, wrong/unknown crit chunk, no null
terminator in text, ... [ ] error messages with line numbers (and version) [ ] errors in state
instead of as return code? [ ] new errors/warnings like suspiciously big decompressed ztxt or iccp
chunk [ ] let the C++ wrapper catch exceptions coming from the standard library and return LodePNG
error codes [ ] allow user to provide custom color conversion functions, e.g. for premultiplied
alpha, padding bits or not, ... [ ] allow user to give data (void*) to custom allocator [X] provide
alternatives for C library functions not present on some platforms (memcpy, ...)
*/
...
*) decoding of PNGs with any color type, bit depth and interlace mode, to a 24- or 32-bit color raw
image, or the same color type as the PNG
*) encoding of PNGs, from any raw image to 24- or 32-bit color, or the same color type as the raw
image
...
*) colorimetric color profile conversions: currently experimentally available in lodepng_util.cpp
only, plus alternatively ability to pass on chroma/gamma/ICC profile information to other color
management system.
...
-if a struct has a corresponding cleanup function, call it before the struct disappears to avoid
memory leaks -if a struct has a corresponding copy function, use the copy function instead of "=".
...
*) LodePNGColorMode info_raw: here you can say what color mode of the raw image (the output) you
want to get
